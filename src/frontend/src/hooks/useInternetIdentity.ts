...
export function InternetIdentityProvider({
  ...
  const login = useCallback(() => {
    ...
    // On mobile devices, passing an empty string for windowOpenerFeatures forces
    // window.open to open a new tab instead of a popup. Android Chrome silently
    // blocks popup windows, so this ensures the sign-in flow always works on mobile.
    const windowOpenerFeatures = isMobileDevice()
      ? ""
